...
class REST {
    ...
    class func loadQuemViu(succes: @escaping([QuemViuDTO]) -> Void) {
        guard let url = URL(string: APPURL.SEE_ALSO) else { return }
        let dataTask = session.dataTask(with: url) { (data: Data?, response: URLResponse?, erro: Error?) in
            if erro == nil {
                guard let response = response as? HTTPURLResponse else { return }
                if response.statusCode == 200 {
                    guard let data =  data else { return }
                    do {
                        let products = try JSONDecoder().decode([QuemViuDTO].self, from: data)
                        succes(products)
                    } catch {
                        print(error.localizedDescription)
                    }
